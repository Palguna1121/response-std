...
func main() {
	...
	filename := findMigrationFor(table, version)
	if filename == "" {
	...
	writeModelFile(table, structDef, version)
}

func findMigrationFor(table, version string) string {
	pattern := fmt.Sprintf("%s/database/migrations/*_create_%s_table.up.sql", version, table)
...
func writeModelFile(table, structContent, version string) {
	outputDir := filepath.Join(version, "models", "entities")
