...
func OwnerMiddleware(resourceIDParam string) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if resourceID != userIDStr {
			...
			if !isAdmin {
				helper.Forbidden(c, "Access denied. You can only access your own resources")
				c.Abort()
				return
			}
		}
	...
	return func(c *gin.Context) {
		...
		if c.Request.Method == "POST" || c.Request.Method == "PUT" || c.Request.Method == "PATCH" {
			...
			if !strings.Contains(contentType, "application/json") &&
				...
				helper.BadRequest(c, "Content-Type must be application/json or multipart/form-data")
				c.Abort()
				return
			}
		}
...
func AdminMiddleware() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if !isAdmin {
			helper.Forbidden(c, "Access denied. Admin privileges required")
			c.Abort()
			return
		}
...
func MultipleRolesMiddleware(roles ...string) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if !hasRole {
			helper.Forbidden(c, "Access denied. Insufficient privileges")
			c.Abort()
			return
		}
...
func IPWhitelistMiddleware(allowedIPs []string) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if !allowed {
			helper.Forbidden(c, "IP address not whitelisted")
			c.Abort()
			return
		}
